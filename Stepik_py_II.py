...
timur = input().strip().lower()
ruslan = input().strip().lower()

# Проверяем, кто победил, или игроки сыграли вничью
if timur == ruslan:
    print("ничья")
...
#
#Формат входных данных
#В первой строке подаётся число n – количество холодильников. В последующих n строках вводятся строки, содержащие латинские строчные буквы и цифры, в каждой строке от 5 до 
#100 символов.
#
#Формат выходных данных
...
s = input()
al = 'абвгдежзиклмнопрстуфхцчшщъыьэюя'
s = s+" запретил букву "
for i in range (0, len(al)):
    if al[i] in s:
        print(s+al[i])
        s= s.replace(al[i], '').lstrip().replace('  ', ' ')
