...
#--182
#На вход программе подается натуральное число n. Напишите программу, использующую списочное выражение, которая создает список содержащий квадраты чисел от 
#1 до n, а затем выводит его элементы построчно, то есть каждый на отдельной строке.
#
#Формат входных данных
#На вход программе подается натуральное число.
#
...
d = input().split()
dd = [int(i)**2 for i in d if ((int(i)**2)%10!=4 and int(i)%2==0)]
print(*dd)


#187 --алгоритм пузырьковой сортировки:
...
#--188  Оптимизация алгоритма "пузырьковой сортировки"
...
#--189 Список четных
#На вход программе подается четное число n  n≥2. Напишите программу, которая 
...
#создает третий список, элементами которого являются суммы соответствующих 
...
#Строка текста является корректным телефонным номером если она имеет 
#формат:
#abc-def-hijk или
#7-abc-def-hijk
...
#На вход программе подается строка текста.
#Формат выходных данных
#Программа должна вывести «YES» если строка является корректным 
#телефонным номером и «NO» в противном случае.
...
nt = input().split('-')
mask = [len(i) for i in nt]
if (mask == [3, 3, 4]) and (''.join(nt).isdigit()):
    print('YES')
elif (mask == [1, 3, 3, 4]) and (''.join(nt).isdigit()) and nt[0]=='7':
    print('YES')
else:
    print('NO')
...
#--199
#Самый длинный
#На вход программе подается строка текста. Напишите 
#программу, использующую списочное выражение, которая находит длину 
...
#Молодежный жаргон
#На вход программе подается строка текста. Напишите 
#программу, использующую списочное выражение, которая преобразует каждое 
...
#--203
Напишите функцию get_distance(x1, y1, x2, y2), вычисляющую расстояние между точками
...
# приводим строку к нижнему регистру и удаляем символы , . ! ? - и пробелы
...
def is_prime(num):   #простое число
    if num == 1:
        return False
    for i in range(2, num):
        if num % i == 0:
...
def is_valid_password(password):    #валидный пароль
...
def convert_to_python_case(text):
    ...
    for i in range(len(text)):
        if i == 0:
            python_case += text[i].lower()
        elif text[i].isupper():
            python_case += '_' + text[i].lower()
        else:
            python_case += text[i]
    return python_case
...
#и находит координаты середины отрезка
...
#1000 рублей за первый товар и 
#120 рублей за каждый последующий товар. Напишите функцию get_shipping_cost(quantity), которая принимает в качестве аргумента натуральное число quantity – количество товаров в заказе и возвращает стоимость доставки.
#
#Примечание. Следующий программный код:
#
...
def get_shipping_cost(quantity):
    ...
    if quantity == 0:
        ...
        return base_cost + additional_cost * (quantity - 1)
...
def is_magic(date):
    ...
    year_last_two_digits = int(year[-2:])
    return int(day) * int(month) == year_last_two_digits
...
# объявление функции
def is_pangram(text):
    # Создаем множество из всех букв алфавита
    alphabet = set('abcdefghijklmnopqrstuvwxyz')
    # Приводим текст к нижнему регистру
    lowercase_text = text.lower().replace(' ','')
    
    # Создаем множество из всех букв текста
    letters = set(lowercase_text)
    # Если множество букв текста равно множеству букв алфавита, то это панграмма
...
#--142
#симулятор десятикратного поочередного подбрасывания монеты. Всякий раз, когда программа имитирует подбрасывание монеты, она должна случайным образом показывать "орла" или "решку".
...
#и просит пользователя угадать это число. Если догадка пользователя больше случайного числа, то программа должна вывести сообщение 'Слишком много, попробуйте еще раз'. Если догадка меньше случайного числа, то программа должна вывести сообщение 'Слишком мало, попробуйте еще раз'. Если пользователь угадывает число, то программа должна поздравить его и вывести сообщение 'Вы угадали, поздравляем!'.
#
